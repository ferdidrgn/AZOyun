...
class _SoccerGameScreenState extends State<SoccerGameScreen>
  ...
  void _onFirebase(DatabaseEvent e) {
    ...
    if (d['status'] == 'finished' && !_finalShown) {
      _finalShown = true;
      Future.delayed(const Duration(milliseconds: 400), _showFinalDialog);
    }
  }
  ...
  void _onPanStart(DragStartDetails d, Size size) {
    ...
    setState(() { _dragStart = norm; _dragCurrent = norm; });
  }
  ...
  void _onPanEnd(DragEndDetails _, Size size) {
    if (_dragStart == null || _dragCurrent == null || !_isMyTurn || _shooting) return;
    ...
    setState(() {
      ...
      _dragStart   = null;
      _dragCurrent = null;
      _gkTargetX   = _rng.nextDouble() * 0.6 + 0.2; // goalkeeper moves
    });
  ...
  Future<void> _checkResult() async {
    // Goal area: y < 0.22, x between 0.28 and 0.72
    final inGoalX = _ball.dx > 0.28 && _ball.dx < 0.72;
    final inGoalY = _ball.dy < 0.22;
    final gkBlocked = (_ball.dx - _gkX).abs() < 0.08 && _ball.dy < 0.25;
  ...
  Future<void> _onGoal() async {
    setState(() { _celebrating = true; _missed = false; });
  ...
  Future<void> _advanceTurn() async {
    ...
    // Only the current player advances the turn
    if (!_isMyTurn) return;
      ...
      body: SafeArea(child: Column(children: [
        ...
        AnimatedContainer(
          ...
          child: Center(child: Text(
            _isMyTurn ? '⚽ Senin sıran — vur!' : '⏳ Rakip vuruyor...',
            ...
            child: Stack(children: [
              CustomPaint(
                ...
                painter: _SoccerFieldPainter(
                  ball:       _ball,
                  gkX:        _gkX,
                  dragStart:  _isMyTurn ? _dragStart : null,
  ...
  void paint(Canvas canvas, Size s) {
    ...
    for (var i = 0; i < 8; i++) {
      if (i.isOdd) canvas.drawRect(
          Rect.fromLTWH(0, s.height * i / 8, s.width, s.height / 8), stripe);
    }

    // ── Centre line ──
    final line = Paint()
      ..color = Colors.white30 ..strokeWidth = 2;
    canvas.drawLine(Offset(0, s.height * .5), Offset(s.width, s.height * .5), line);
    canvas.drawCircle(Offset(s.width * .5, s.height * .5), 40, line..style = PaintingStyle.stroke);
    ...
    canvas.drawRect(Rect.fromLTWH(goalX, 0, goalW, goalH), goalPaint);

    // Net pattern
    final netPaint = Paint()..color = Colors.white24..strokeWidth = 1;
    for (var x = goalX; x < goalX + goalW; x += 18) {
      canvas.drawLine(Offset(x, 0), Offset(x, goalH), netPaint);
    }
    for (var y = 0.0; y < goalH; y += 14) {
      canvas.drawLine(Offset(goalX, y), Offset(goalX + goalW, y), netPaint);
    }

    // ── Goalkeeper ──
    final gkCX = gkX * s.width;
    final gkCY = goalH + 10;
    canvas.drawCircle(Offset(gkCX + 2, gkCY + 3), 20,
    ...
    canvas.drawCircle(Offset(gkCX - 5, gkCY - 4), 3, eye);
    canvas.drawCircle(Offset(gkCX + 5, gkCY - 4), 3, eye);
    // Gloves
    final glove = Paint()..color = Colors.yellow.shade800;
    canvas.drawCircle(Offset(gkCX - 22, gkCY), 8, glove);
    canvas.drawCircle(Offset(gkCX + 22, gkCY), 8, glove);
    ...
    if (dragStart != null && dragCurrent != null) {
      ...
      final power = (sqrt(dx*dx + dy*dy) / (s.width * 0.3)).clamp(0.0, 1.0);
      final tx    = bx + dx * 1.3;
      final ty    = by + dy * 1.3;

      canvas.drawLine(Offset(bx, by), Offset(tx, ty),
          Paint()
            ..color = Color.lerp(Colors.white.withOpacity(0.8),
                Colors.redAccent.withOpacity(0.8), power)!
            ..strokeWidth = 3.5 ..strokeCap = StrokeCap.round);

      for (var i = 1; i <= 5; i++) {
        canvas.drawCircle(
            Offset(bx + dx * 1.3 * i / 6, by + dy * 1.3 * i / 6), 5,
    ...
    final bx = ball.dx * s.width;
    final by = ball.dy * s.height;
    canvas.drawCircle(Offset(bx + 2, by + 3), 16,
        Paint()..color = Colors.black38
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8));
    canvas.drawCircle(Offset(bx, by), 15, Paint()..color = Colors.white);
    // Pentagons
    final pent = Paint()..color = Colors.black87;
    for (var i = 0; i < 5; i++) {
      final a = i * 2 * pi / 5 - pi / 2;
      canvas.drawCircle(
          Offset(bx + cos(a) * 9, by + sin(a) * 9), 4, pent);
    }
    canvas.drawCircle(Offset(bx, by), 4, pent);
    // Highlight
    canvas.drawCircle(Offset(bx - 5, by - 5), 5,
