...
class VampireLobbyScreen extends StatefulWidget {
  ...
  @override State<VampireLobbyScreen> createState() => _VampireLobbyScreenState();
}
...
class _VampireLobbyScreenState extends State<VampireLobbyScreen> {
  ...
  Future<void> _askName() async {
    final name = await showNameDialog(context, current: _playerName, accentColor: Colors.deepPurple);
            ...
            AZFrostCard(opacity: 0.08, child: Column(
                ...
                children: const [
                  ...
                  Text(
                    ...
                    '• Vampirler yoksa Köylüler, sayı eşitlenince Vampirler kazanır',
...
class VampireRoomScreen extends StatefulWidget {
  ...
  @override State<VampireRoomScreen> createState() => _VampireRoomScreenState();
}
...
class VampireGameScreen extends StatefulWidget {
  const VampireGameScreen({super.key, required this.roomId,
      required this.myKey, required this.myName});
  final String roomId, myKey, myName;
  @override State<VampireGameScreen> createState() => _VampireGameScreenState();
}
...
class _VampireGameScreenState extends State<VampireGameScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final targets = _players.entries
        .where((e) => e.key != widget.myKey && e.value['alive'] == true).toList();
          ...
          child: Column(children: [
            ...
            Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text((isNight ? 'GECE' : 'GÜNDÜZ') + _dayLabel(),
                    style: const TextStyle(color: Colors.white,
            ...
            Expanded(child: Builder(builder: (_) {
              if (!_alive) return const AZFrostCard(child: Center(child: Text(
                '☠️ Elendin\nİzlemeye devam edebilirsin',
                textAlign: TextAlign.center,
                style: TextStyle(color: Colors.white, fontSize: 16))));
              if (_voted) return AZFrostCard(child: Center(child: Column(
              ...
              if (isNight && !_isVampire) return AZFrostCard(
                  child: const Text('🌙 Uyku vakti...\nVampirler kurban seçiyor.',
                      textAlign: TextAlign.center,
                      style: TextStyle(color: Colors.white, fontSize: 16)));
